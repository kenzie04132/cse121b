...
function gameStart(){
	...
	navBar.className = 'nav_bar';
		...
		giveUpBtn.addEventListener("click", function quitClick(event) {
			if(gameFin == 0){
				notification.innerText = 'The word was ' + chosenWord + '. Press Enter On Your Keyboard';
				gameOver();
			}
		});
...
function submitWord(wordRow, keyPress){
	if(nextRowBlock > 0 && nextRowBlock % 5 == 0){
		...
		if(wordlist.includes(word)){
            ...
            //Displays helpful text on whether you win or lose the game, as well as if the user makes an incorrect input
			if(score === 5){
				notification.innerText = 'Great Job! Press Enter On Your Keyboard.';
				gameOver();
			}
